...
const SignupPage = () => {
  ...
  useEffect(() => {
    const handleGoogleCallback = async () => {
      ...
      const isGoogleCallback = urlParams.get('google_auth') === 'true' ||
      ...
      if (isGoogleCallback) {
        ...
        try {
          ...
          // Fetch current user to check if authentication was successful
          const response = await fetch(`${apiBase}/api/v1/users/current-user`, {
            method: 'GET',
            credentials: 'include',
            headers: {
          ...
          if (response.ok) {
            ...
            if (result.data) {
              ...
              // Call signup/signin with Google auth
              await signup({
                email: result.data.email,
                fullName: result.data.fullName,
                password: "google-auth",
                phoneNumber: result.data.phoneNumber || 0,
              });
              ...
              const redirectUrl = result.data.role === 'admin' ? '/admin' : from;
              navigate(redirectUrl, { replace: true });
            }
          } else {
            // If not authenticated, just clean URL
            window.history.replaceState({}, document.title, window.location.pathname);
          }
        } catch (error) {
